...
CakePHP 的安装简单方便. 最低要求是
Web服务器和一份CakePHP, 就是这样! 本章重点
主要是建立在Apache（因为安装很简单）,
CakePHP将运行在各种Web服务器如nginx的，lighthttpd，或
微软的IIS。

必要条件
============

- HTTP Server. 例如: Apache. 有MOD \ _rewrite是首选，但
并不是必要条件.
- PHP 5.4.16 或更高版本.
- mbstring 扩展
- intl 扩展
...
.. 备注::
    ...
    在 XAMPP 和 WAMP, mbstring 扩展是默认工作的.
